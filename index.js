...
bot.on('text', (ctx) => {
    ...
    .then(async (message) => {
      ...
      if (cid == null) {
        ...
        // encodeURIComponent()
        ctx.replyWithPhoto({ url: `https://nbcovtest.onrender.com/prodimage?img=${encodeURIComponent(resp.data.normal.image)}&titel=${encodeURIComponent(`المنتج ${resp.data.normal.name}`)}&normal=${encodeURIComponent(resp.data.normal.discountPrice !== "none" ? resp.data.normal.discountPrice : resp.data.normal.price)}&points=${encodeURIComponent(resp.data.points.total)}&superd=${encodeURIComponent(resp.data.super.price)}&limited=${encodeURIComponent(resp.data.limited.price)}&shipping=${encodeURIComponent(resp.data.normal.discountPrice !== "free" ? resp.data.normal.discountPrice + "$" : "مجاني")}&shippingcomp=${encodeURIComponent(resp.data.normal.shippingInfo.type)}&shippingest=${encodeURIComponent(resp.data.normal.shippingInfo.deliverRange)}&store=${encodeURIComponent(resp.data.normal.store)}` },
      {
